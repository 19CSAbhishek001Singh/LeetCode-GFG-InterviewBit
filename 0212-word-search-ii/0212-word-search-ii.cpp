...
public:
    ...
    //useful to move in 4 directions
...
vector<string> findWords(vector<vector<char>>& board,
        ...
        for(int i=0;i<rows;i++) {
            ...
            //if a starting letter of a wod exists in the words dict
    ...
    void checkWord(TrieNode* curr,vector<vector<char>>& board,
                   int i,int j,vector<string>& res,string s) {
        ...
        if(curr->endsHere>0) {
            ...
            res.push_back(s);
            curr->endsHere-=1;
    //since  not to push again the same word from other possibility
        }
        char orig=board[i][j];
        board[i][j]='#';
        for(int k=0;k<4;k++) {
